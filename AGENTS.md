...
After a complex implementation, use at most one reviewer sub-agent pass per task slice. Treat reviewer feedback critically, fix only clear issues, and if feedback becomes ambiguous/speculative or opens new branches, stop and ask the user with current state + suggested options instead of looping.
