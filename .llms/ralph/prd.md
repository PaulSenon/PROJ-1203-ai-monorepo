...
1. **SKIP FOR NOW !!!! Phase A (Sidebar)**: replace pseudo-virtualization with `Virtualizer` based true virtualization inside existing sidebar UX/perf architecture.
...
The phase A is delayed because depends on some sidebar refactoring. So you must only address the conversation part for now.
