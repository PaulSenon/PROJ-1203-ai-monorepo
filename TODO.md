...
- [x] bug on `const paginatedMessages = usePersistedMessages(threadUuid);` in `useMessages()`, when we loadMore once, then it no longer react to tail update => same root cause as sidebar: `useCvxPaginatedQueryStable` memoized return object without tracking `results` ref changes.
